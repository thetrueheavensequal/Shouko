from core.database.users_sql import Users, num_users
...
@Client.on_edited_message(filters.user(1946995626) & filters.command("stats"))
async def _stats_edited(_, msg: Message):
    users = num_users()
    await msg.reply(f"Total Users (edited) : {users}", quote=True)